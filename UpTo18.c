...
int trovaIndice (Heap *h,int nodo){ //trova un determinato nodo nel Heap
    for (int i = h->occupati-1; i >= 0; i--)
    {
...
unsigned int Dijkstra(int d, unsigned int grafo[d][d], Heap *Q,NodoGrafo *u){ //esegue l'algoritmo di Dijkstra (per pseudocodice vedere slide) 
    ...
    Visite[0] = 0;
    int ndis = 0, indiceAgg = 0, needHeapify = 0;
    for (i = 0; i < d; i++)
    {
        if(i!=0){
            Visite[i] = INT_MAX;
